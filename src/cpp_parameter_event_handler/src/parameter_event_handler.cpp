...
public:
  SampleNodeWithParameters() : Node("node_with_parameters") {
    ...
    auto cb2 = [this](const rclcpp::Parameter& p) {
      RCLCPP_INFO(
        this->get_logger(),
        "cb2: Received an update to parameter \"%s\" of type %s: \"%.021f\"",
        p.get_name().c_str(),
        p.get_type_name().c_str(),
        p.as_double()
      );
    ...
    auto event_cb = [this](const rcl_interfaces::msg::ParameterEvent& p_event) {
      RCLCPP_INFO(
        this->get_logger(),
      ...
      for (const auto& p : p_event.changed_parameters) {
        RCLCPP_INFO(
          this->get_logger(),
